...
use GeneratorRemake\object\Dungeons;
use GeneratorRemake\populator\VariableAmountPopulator;
...
class Mineshaft extends VariableAmountPopulator
{
    const DIR_XPLUS = 0;
    const DIR_XMIN = 1;
    const DIR_ZPLUS = 2;
    ...
    public function populate(ChunkManager $level, $chunkX, $chunkZ, Random $random)
    {
        ...
        $startingPath = $random->nextBoundedInt(8);
        $this->maxPath = $random->nextBoundedInt(100) + 100;
                ...
                switch ($dir) {
                    case self::DIR_XPLUS :
                        ...
                        if ($random->nextBoundedInt(30) == 0) {
                            ...
                            if ($direction2 == 1)
                                $direction2 = 4;
                            $v = new Vector3($x + $direction2, $y, $z + $direction);
                            Dungeons::prepareChest($v->getX(), $v->getY(), $v->getZ());
                        }
                    ...
                    case self::DIR_ZPLUS :
                        ...
                        if ($random->nextBoundedInt(30) == 0) {
                            ...
                            if ($direction2 == 1)
                                $direction2 = 4;
                            $v = new Vector3($x + $direction, $y, $z + $direction2);
                            Dungeons::prepareChest($v->getX(), $v->getY(), $v->getZ());
                        }
                        if ($random->nextRange(0, 15) !== 0)
