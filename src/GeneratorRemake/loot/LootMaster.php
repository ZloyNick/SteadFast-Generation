...
namespace GeneratorRemake\loot;
...
class LootMaster
{
    ...
    public static function placeAll(ChunkManager $level)
    {
        ...
        /** @var Level $level */
        $stream = file_get_contents(self::getDataFolder() . "processingLoot.txt");
        ...
        foreach ($data as $pos) {
            ...
            if ($tile != null && $tile instanceof \pocketmine\tile\Chest)
                self::fillChest($tile);
    ...
    public static function getRandomItems(): array
    {
        $items = [];
        $rand = static::$loot;
